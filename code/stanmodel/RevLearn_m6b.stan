...
data {
  ...
  matrix<lower=1,upper=2>[nTrials,4] otherChoice2_V[nSubjects];
  matrix<lower=1,upper=2>[nTrials,4] otherChoice2_R[nSubjects];
  matrix<lower=1,upper=2>[nTrials,4] otherChoice2_L[nSubjects];  
...
parameters {
  ...
  vector[3] disc_mu_pr; // discounting factor
  vector<lower=0>[3] disc_sd;
  cholesky_factor_corr[3] disc_L;
...
transformed parameters {
  ...
  matrix[nSubjects,3] disc_eff;
  matrix<lower=0,upper=1>[nSubjects,3] disc;
  ...
  disc_eff = rep_matrix(disc_mu_pr', nSubjects) + disc_raw * diag_pre_multiply(disc_sd, disc_L)';
  /* This is:
...
model {
  ...
  disc_mu_pr ~ std_normal();
  disc_sd ~ normal(0,0.2);
  disc_L ~ lkj_corr_cholesky(2); 
  ...
  for (s in 1:nSubjects) {    
    ...
    matrix[3,4] disc_mat_V;
    matrix[3,4] disc_mat_R;
    matrix[3,4] disc_mat_L;
    ...
    for (t in 1:nTrials) {
      ...
      if (t == 1) {
        ...
        otherReward_vec_V = wOthers[s,t] .* otherReward2_V[s,t] + wOthers[s,t-1] .* otherReward2_V[s,t-1] * disc[s,1];
        otherReward_vec_V = otherReward_vec_V * 0.5;
      } else {
        for ( j in 1:num_elements(pwr)) {
          disc_mat_V[j] = rep_row_vector(pow(disc[s,1], pwr[j]), 4);
        }
        otherReward_vec_V = rep_row_vector(1.0, 3) * (disc_mat_V .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2_V[s], t-2, 1, 3, 4));
        otherReward_vec_V = otherReward_vec_V * (1.0/3.0);
      }
      ...
      if (t == 1) {
        ...
        otherReward_vec_R = wOthers[s,t] .* otherReward2_R[s,t] + wOthers[s,t-1] .* otherReward2_R[s,t-1] * disc[s,2];
        otherReward_vec_R = otherReward_vec_R * 0.5;   // --> to make them a [0 1] range
      } else {
        for ( j in 1:num_elements(pwr)) {
          disc_mat_R[j] = rep_row_vector(pow(disc[s,2], pwr[j]), 4);
        }
        otherReward_vec_R = rep_row_vector(1.0, 3) * (disc_mat_R .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2_R[s], t-2, 1, 3, 4));
        otherReward_vec_R = otherReward_vec_R * (1.0/3.0);
      }
      ...
      if (t == 1) {
        ...
        otherReward_vec_L = wOthers[s,t] .* otherReward2_L[s,t] + wOthers[s,t-1] .* otherReward2_L[s,t-1] * disc[s,3];
        otherReward_vec_L = otherReward_vec_L * 0.5;
      } else {
        for ( j in 1:num_elements(pwr)) {
          disc_mat_L[j] = rep_row_vector(pow(disc[s,3], pwr[j]), 4);
        }
        otherReward_vec_L = rep_row_vector(1.0, 3) * (disc_mat_L .* block(wOthers[s], t-2, 1, 3, 4) .* block(otherReward2_L[s], t-2, 1, 3, 4));
        otherReward_vec_L = otherReward_vec_L * (1.0/3.0);
      }
...
generated quantities {
  ...
  vector<lower=-1,upper=1>[3] disc_mu;
  matrix[3,3] lr_corr; // correlation coefficient
  matrix[3,3] disc_corr;
  ...
  disc_mu = inverse(M) * Phi_approx(M * disc_mu_pr);
  lr_corr = multiply_lower_tri_self_transpose(lr_L);
  disc_corr = multiply_lower_tri_self_transpose(disc_L);
    ...
    for (s in 1:nSubjects) {    
      ...
      for (t in 1:nTrials) {
        ...
        if (t == 1) {
          ...
          otherReward_vec_R = wOthers[s,t] .* otherReward2_R[s,t] + wOthers[s,t-1] .* otherReward2_R[s,t-1] * disc[s,2];
          otherReward_vec_R = otherReward_vec_R * 0.5;
        } else {
          for ( j in 1:num_elements(pwr)) {
            disc_mat_R[j] = rep_row_vector(pow(disc[s,2], pwr[j]), 4);
          }
